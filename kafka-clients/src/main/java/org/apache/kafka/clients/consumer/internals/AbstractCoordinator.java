...
public abstract class AbstractCoordinator implements Closeable {
    ...
    /**
     * 获取本地成员支持的当前协议列表及其关联的元数据。
     * 协议在列表中的顺序表示协议的首选项(第一个条目是首选项)。
     * 协调器在选择生成协议时将此首选项考虑在内(通常，只要所有成员都支持并且对首选项没有异议，就会选择更首选的协议)。
    ...
    /**
     * 加入这个小组，把作业交给下一代。
     * 此函数处理JoinGroup和SyncGroup，如果协调器选择leader，则将其委托给{@link #performAssignment(String, String, Map)}。
     * @return 一个封装了组长返回的任务的请求future
     */
